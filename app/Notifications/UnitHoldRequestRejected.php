...
class UnitHoldRequestRejected extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {         
        return OneSignalMessage::create()
            ->subject("សម្នើរព្យួរត្រូវបានបដិសេដ")
            ->body("សម្នើរព្យួរលើអចលនទ្រព្យ ".$this->unit_hold_request->unit->code." ត្រូវបានបដិសេដ")
            ->icon('ic_logo_gray')
            ->setData('notification_id', $this->id)
            ->setData('id', $this->unit_hold_request->id)
            ->setData('type', "unit_hold_request_rejected");
    }
}

...
class UnitDepositRequestApproved extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(UnitDepositRequest $unit_deposit_request, $user_role = "")
    {
        ...
        $this->unit_deposit_request->loadMissing(['unit']);
    ...
    {         
        return OneSignalMessage::create()
            ->subject("សម្នើរកក់ប្រាក់ត្រូវបានអនុម័ត")
            ->body("សម្នើរកក់ប្រាក់លើអចលនទ្រព្យ ៖ ".$this->unit_deposit_request->unit->code." ត្រូវបានអនុម័ត")
            ->icon('ic_logo_gray')
            ->setData('notification_id', $this->id)
            ->setData('id', $this->unit_deposit_request->id)
            ->setData('type', "unit_deposit_request_approved");
    }
}

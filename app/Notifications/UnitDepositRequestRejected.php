...
class UnitDepositRequestRejected extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {         
        return OneSignalMessage::create()
            ->subject("សម្នើរកក់ប្រាក់ត្រូវបានបដិសេដ")
            ->body("សម្នើរកក់ប្រាក់លើអចលនទ្រព្យ ".$this->unit_deposit_request->unit->code." ត្រូវបានបដិសេដ")
            ->icon('ic_logo_gray')
            ->setData('notification_id', $this->id)
            ->setData('id', $this->unit_deposit_request->id)
            ->setData('type', "unit_deposit_request_rejected");
    }
}

...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <form method="GET" action="{{ route('admin.user.all') }}" name="user-search-from" novalidate="novalidate" autocomplete="false">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            User List : <a href="{{route('admin.user.create')}}" class="btn btn-primary btn-sm ">Create New User</a>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="input-group input-group-sm col-md-3">
                                ...
                                <select class="form-control" name="role" id="role_select">
                                    ...
                                    <option value="{{ $role->id }}" {{ Request::query('role') == $role->id ? 'selected' : '' }}>{{ str_replace('_',' ', title_case($role->name)) }}</option>
                                    @endforeach                                    
                                </select>
                            </div>                          
                    ...
                    <table class="table table-hover table-action mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="action-td">
                                    <a href="{{ route('admin.user.show',['id'=>$user->id]) }}" class="title" >{{ $user->name }}</a>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left" aria-labelledby="zdropdownMenuButton">
                                            <a href="{{ route('admin.user.show', [ 'id' => $user->id ] ) }}" class="dropdown-item">
                                                {{__("View")}}
                                            </a>
                                            <a href="{{ route('admin.user.edit', [ 'id' => $user->id ] ) }}" class="dropdown-item">
                                                {{__("Edit")}}
                                            </a>
                                            <a href="{{ route('admin.user.password.show', ['id' => $user->id]) }}" class="dropdown-item"> {{ __('Reset Password') }}</a>
                                            <a href="{{ route('admin.user.token.show', ['id' => $user->id]) }}" class="dropdown-item">{{ __('Login Devices') }}</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>

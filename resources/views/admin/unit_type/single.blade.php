                ...
                <div class="card-body bg-light position-relative">
                    ...
                    <div class="btn-group btn-group-action-absolute">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a href="{{ route('admin.contract_templates.preview', [ 'template_path' => strtolower($unit_type->contractTemplate->template_path), 'unit_type_id' => $unit_type->id, 'version'=>'v2']) }}" class="dropdown-item" target="_blank">{{ __("View Contract Template") }}</a>
                ...
                <div class="col-12">
                    <div class="d-flex flex-column bg-white border">
                        <div class="text-center bg-primary p-3 ">
                            <a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id]) }}" class="font-weight-bold display-4 d-block text-white">{{ $unit_type->units_count ?? 'N/A' }}</a>
                            <small class="text-white">{{ __('Total Units') }}</small>
                        </div>
                        <div class="d-flex flex-column flex-sm-row">
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id, 'status' => 'AVAILABLE']) }}">{{ $unit_type->available_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Available') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id, 'status' => 'UNAVAILABLE']) }}">{{ $unit_type->unavailable_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Unavailable') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id, 'status' => 'HOLD']) }}">{{ $unit_type->hold_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Hold') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id, 'status' => 'DEPOSIT']) }}">{{ $unit_type->deposit_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Deposit') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('admin.projects.units.index', [ 'id' => $unit_type->project_id, 'unit_type_id' => $unit_type->id, 'status' => 'CONTRACT']) }}">{{ $unit_type->contract_unit_count ?? 'N/A' }}</a></h4>

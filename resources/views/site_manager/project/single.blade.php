...
<div class="container">
    <div class="row">
        <div class="col-lg-4">
            <div class="card mb-3">
                ...
                <div class="card-body position-relative">
                    <h5><a href="{{ route('site_manager.projects.show', ['id' => $project->id]) }}">{{ $project->name }}</a></h5>
                    ...
                    <div class="btn-group btn-group-action-absolute">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            @can('update-project')
                            <a href="{{ route('site_manager.projects.edit', [ 'id' => $project->id]) }}" class="dropdown-item">{{ __("Edit") }}</a>
                            @endcan

                            @can('view-master-plan-project')
                            <a href="{{ route('site_manager.projects.master_plan', [ 'id' => $project->id]) }}" class="dropdown-item">{{ __("View Master Plan") }}</a>
                            @endcan
                        </div>
                    </div>
                </div>    
        ...
        <div class="col-lg-8">
                ...
                <div class="col-12">
                    <div class="d-flex flex-column bg-white border">
                        <div class="text-center bg-primary p-3 ">
                            <a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id]) }}" class="font-weight-bold display-4 d-block text-white">{{ $project->units_count ?? 'N/A' }}</a>
                            <small class="text-white">{{ __('Total Units') }}</small>
                        </div>
                        <div class="d-flex flex-column flex-sm-row">
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'AVAILABLE']) }}">{{ $project->available_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Available') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'UNAVAILABLE']) }}">{{ $project->unavailable_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Unavailable') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'HOLD']) }}">{{ $project->hold_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Hold') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'DEPOSTI']) }}">{{ $project->deposit_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Deposit') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'CONTRACT']) }}">{{ $project->contract_unit_count ?? 'N/A' }}</a></h4>
                                <small class="text-muted">{{ __('Contract') }}</small>
                            </div>
                            <div class="flex-fill p-3 text-center border bg-grey">
                                <h4 class="mb-0 font-weight-bold"><a href="{{ route('site_manager.projects.units.index', ['project_id' => $project->id, 'status' => 'HANDOVERED']) }}">{{ $project->handovered_unit_count ?? 'N/A' }}</a></h4>
            ...
            @else
                @include('template.unit_types.carousel', ['unit_types' => $project->unitTypes, 'route_prefix' => 'site_manager'])
            @endif
            ...
            @else
                @include('template.zones.carousel', ['zones' => $project->zones, 'route_prefix' => 'site_manager'])
            @endif                   
        </div>
    </div>
</div>

                ...
                <div class="col">
                    <ul class="yk-nav nav-small">
                        <li class="nav-item active">
                            <a href="{{ route('handover_officer.unit_handovers.index') }}" class="nav-link {{ Request::query('view') ? '' : 'active' }}">Mine</a>
                        </li> 
                        <li class="nav-item">
                            <a href="{{ route('handover_officer.unit_handovers.index', ['view' => 'all']) }}" class="nav-link {{ Request::query('view') == 'all' ? 'active' : ''}}">All</a>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('handover_officer.unit_handovers.index', ['view' => 'pending_my_approval']) }}" class="nav-link {{ Request::query('view') == 'pending_my_approval' ? 'active' : ''}}">Need My Approval</a>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div class="pr-3 align-self-center">
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left">
                            ...
                            <a href="{{route('handover_officer.unit_handovers.show',['id'=>$handover->id])}}" class="dropdown-item">View</a>
                            @endif
                            @if( Auth::user()->can('update-handover-request') )
                                @if($handover->status == 'Draft')  
                                    <a href="{{route('handover_officer.unit_handovers.edit',['id'=>$handover->id])}}" class="dropdown-item">Edit</a>
                                @endif
                            @endif
                        </div>
                    </div>
